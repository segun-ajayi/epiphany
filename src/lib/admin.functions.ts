...
export const adminLogin = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (!expected) {
      return {
        ok: false as const,
        error: "ADMIN_PASSWORD is not configured on this environment.",
      };
    }
...
export const adminUpdateMessageStatus = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    await requireAdmin();
    const m = await store.updateMessageStatus(data.id, data.status);
...
export const adminSendBroadcast = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    let sent = 0;
    let failed = 0;
    let provider: "resend" | "console" | "mixed" = getEnv("RESEND_API_KEY") ? "resend" : "console";
