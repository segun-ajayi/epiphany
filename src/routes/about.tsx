...
export const Route = createFileRoute("/about")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Our story, mission, beliefs, and leadership team at Anglican Church of Epiphany in Houston, Texas." },
      ...
      <section className="container-page py-20 md:py-28">
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <Card className="p-10 bg-primary text-primary-foreground">
            ...
            <h2 className="mt-3 font-display text-3xl">A parish family formed by scripture, sacrament, and love.</h2>
      ...
      <section className="bg-secondary/50 py-20 md:py-28">
        <div className="container-page">
          ...
          <ol className="mt-16 relative max-w-3xl mx-auto">
            <span className="absolute left-4 md:left-1/2 top-0 bottom-0 w-px bg-border" aria-hidden />
            {HISTORY.map((h, i) => (
              <li key={h.year} className={`relative grid md:grid-cols-2 gap-6 mb-12 md:mb-16 ${i % 2 === 1 ? "md:[&>*:first-child]:order-2" : ""}`}>
                ...
                <span className="absolute left-2.5 md:left-1/2 top-2 -translate-x-1/2 size-3 rounded-full bg-gold ring-4 ring-background" aria-hidden />
              </li>
            ))}
          </ol>
        </div>
      ...
      <section className="container-page py-20 md:py-28">
        <div className="grid lg:grid-cols-2 gap-12">
          <div>
            ...
            <h2 className="mt-3 font-display text-3xl md:text-5xl">Rooted in scripture, formed by tradition</h2>
          ...
          <Accordion type="single" collapsible defaultValue="b0" className="w-full">
            {BELIEFS.map((b, i) => (
              <AccordionItem key={b.title} value={`b${i}`}>
                <AccordionTrigger className="font-display text-lg text-left">{b.title}</AccordionTrigger>
                <AccordionContent className="text-muted-foreground leading-relaxed">{b.body}</AccordionContent>
              </AccordionItem>
            ))}
          </Accordion>
        </div>
          ...
          <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {LEADERSHIP.map((l) => (
              <Card key={l.email} className="overflow-hidden text-center">
                <div className="aspect-square bg-gradient-to-br from-primary to-[#152a4a] grid place-items-center text-primary-foreground">
                ...
                <CardContent className="p-6">
                  ...
                  <a href={`mailto:${l.email}`} className="mt-4 inline-flex items-center gap-2 text-sm text-primary hover:text-gold transition">
...
export function PageHero({ eyebrow, title, subtitle, image }: { eyebrow: string; title: string; subtitle?: string; image: string }) {
  return (
    <section className="relative -mt-16 md:-mt-20 min-h-[60vh] flex items-end overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-[#0F172A] via-[#0F172A]/70 to-[#0F172A]/40" aria-hidden />
