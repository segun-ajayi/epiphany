...
export const Route = createFileRoute("/give")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Support the mission of Anglican Church of Epiphany. Give a one-time or recurring gift online." },
...
const DESIGNATIONS = ["General Fund", "Missions", "Building Fund", "Youth Ministry", "Outreach Programs"];
...
function GivePage() {
  ...
  const handleDonation = async (e: React.FormEvent) => {
    ...
    alert(`Thank you! Donation of $${custom || amount} (${type}) to ${designation} – payment integration pending.`);
  };

  return (
    <>
      <PageHero eyebrow="Generosity" title="Support the Mission" subtitle="Your generosity helps us serve our community and share God's love." image={IMAGES.bible} />

      <section className="container-page py-16">
        <div className="grid lg:grid-cols-[1.4fr_1fr] gap-10">
          <form onSubmit={handleDonation} className="rounded-3xl border border-border bg-card p-8 md:p-10 space-y-8 shadow-elegant">
            <div>
              <h2 className="font-display text-2xl">Donation type</h2>
              <div className="mt-3 grid grid-cols-2 gap-3">
                {(["one","recurring"] as const).map((t) => (
                  <button key={t} type="button" onClick={() => setType(t)} className={`rounded-xl border p-4 text-left transition ${
                    type === t ? "border-gold bg-gold/10" : "border-border hover:bg-accent"
                  }`}>
                    <p className="font-display text-lg">{t === "one" ? "One-Time" : "Recurring"}</p>
                    <p className="text-xs text-muted-foreground">{t === "one" ? "A single gift" : "Monthly or weekly"}</p>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="mt-3 grid grid-cols-3 sm:grid-cols-5 gap-2">
                {AMOUNTS.map((a) => (
                  <button key={a} type="button" onClick={() => { setAmount(a); setCustom(""); }} className={`rounded-lg border py-3 font-medium transition ${
                    amount === a && !custom ? "border-gold bg-gold/15 text-foreground" : "border-border hover:bg-accent"
                  }`}>
                    ${a}
                  </button>
                ))}
              ...
              <div className="mt-3 relative max-w-xs">
                ...
                <Input value={custom} onChange={(e) => { setCustom(e.target.value); setAmount(""); }} placeholder="Custom amount" inputMode="numeric" className="pl-7" />
              </div>
            </div>

            <div>
              <h2 className="font-display text-2xl">Designation</h2>
              <select value={designation} onChange={(e) => setDesignation(e.target.value)} className="mt-3 w-full h-10 rounded-md border border-input bg-background px-3 text-sm">
            ...
            <div>
              ...
              <div className="mt-3 grid sm:grid-cols-2 gap-3">
                ...
                <Input required type="email" placeholder="Email" aria-label="Email" className="sm:col-span-2" />
                <Input type="tel" placeholder="Phone" aria-label="Phone" className="sm:col-span-2" />
              </div>
            </div>
